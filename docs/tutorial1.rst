...
This will inform the :code:`Agent` of another client and build a gRPC client in order to perform the required consensus algorithm tasks throughout its availability. Once an :code:`Agent` discovers another, an automatic voting round is started and a leader is elected. 
