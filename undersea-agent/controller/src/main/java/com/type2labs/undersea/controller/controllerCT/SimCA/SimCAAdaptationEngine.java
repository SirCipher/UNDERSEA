...
import com.type2labs.undersea.controller.controllerCT.simplex.implementation.*;
...
public class SimCAAdaptationEngine {
    ...
    // previous X invocations
    {
        ...
        if (currentOperatingMode == OperatingPhase.IDENTIFICATION) {
            ...
            goals.get(identifiedConcern).dataForModelIdentification.put(goals.get(identifiedConcern).controlDecision,
                    measured[identifiedConcern]);
            ...
            //goal identified
            {
                ...
                goals.get(identifiedConcern).kalman_noise_covariance =
                        goals.get(identifiedConcern).kalman_process_noise_variace + goals.get(identifiedConcern).kalman_measuremnt_noise_variance;
            ...
            for (int l = 0; l < N_concerns; l++) {
                ...
                if (simplexError == 0)//only if simplex produced a solution
                {
                    ...
                    if (goals.get(l).lastControls.size() >= SLOPE_MEASUREMENTS_DISTANCE) {
                        measured_slope =
                                (goals.get(l).lastObservedValues.getLast() - goals.get(l).lastObservedValues.getFirst())
                    ...
                    if (measured_slope.isInfinite() || measured_slope.isNaN()) {
                        ...
                        goals.get(l).kalman_noise_covariance =
                                goals.get(l).kalman_process_noise_variace + goals.get(l).kalman_measuremnt_noise_variance;
                    }
                    ...
                    goals.get(l).kalman_noise_covariance =
                            goals.get(l).kalman_noise_covariance + goals.get(l).kalman_process_noise_variace;
                    Double kalmanK =
                            goals.get(l).kalman_noise_covariance / (goals.get(l).kalman_noise_covariance + goals.get(l).kalman_measuremnt_noise_variance);
                ...
                //controller
                goals.get(l).controlDecision = calculateControlSignal(goals.get(l).oldControlDecision,
                        goals.get(l).error, goals.get(l).pole, l);
        ...
        for (int l = 0; l < N_concerns; l++) {
            ...
            //System.out.println("oldcD"+ l + ": " + goals.get(l).oldControlDecision + "; error: "+ goals.get(l)
            // .error + " pole:" + goals.get(l).pole);
    ...
    private Double nextIdentificationControlDecision(int l) {
        double controlDec =
                (goals.get(l).maxValue - goals.get(l).minValue) * goals.get(l).nextIdentitificationControlDecision + goals.get(l).minValue;
        goals.get(l).nextIdentitificationControlDecision =
                goals.get(l).nextIdentitificationControlDecision + IDENTIFICATION_PROCEDURE_STEP;
    ...
    //e.g., goal 1 = speed, goal 2 = energy consumption, then getSensorProperty(1,3) returns speed of UUV with sensor
    // 3 turned on
    {
          ...
          /* // uncomment to see the equations
            ...
            for (int i = 0; i < N_services; i++) {
                ...
                if (solution.getCoefficients().getEntry(i) != 0) {
                    System.out.print(solution.getCoefficients().getEntry(i) + "*S" + (i + 1));// + " + " +
                    // table[3][sol] + "*x" + x4);
                    if (i != (N_services - 1))
                        System.out.print(" + ");
                }
            ...
            System.err.println("Unbounded Solution");
            simplexError = 1;
