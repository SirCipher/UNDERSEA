...
bool Sensor::handleChange(string value) {
    ...
    if (v.empty() || v.size() != 3) {
        reportConfigWarning("Problem with configuring 'CHANGE =" + value + "': expected 3 variables but received " +
                            intToString(v.size()));
        return false;
    }
    ...
    for (vector<string>::iterator it = v.begin(); it != v.end(); it++) {
        if (!isNumber(*it, false)) {
            reportConfigWarning(
                    "Problem with configuring 'CHANGE =" + value + "': expected double but received " + *it);
            return false;
        }
    }
    ...
    if (atof(v.at(0).c_str()) > atof(v.at(1).c_str())) {
        reportConfigWarning("Problem with configuring 'CHANGE =" + value + "': starting time (" + v.at(0) +
                            ") is after ending time (" + v.at(1) + ") "
